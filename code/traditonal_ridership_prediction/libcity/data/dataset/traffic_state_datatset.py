...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.batch_size = self.config.get('batch_size', 64)  # default=64
        ...
        print('self.parameters_str', self.parameters_str)
    ...
    def _load_dyna_3d(self, filename):
        ...
        # num_complete = total_rows // len_time * len_time
        # df = df.iloc[:num_complete]

        # print('1',len_time)
        data = []
        for i in range(0, df.shape[0], len_time):
            data.append(df[i:i + len_time].values)
            # print(len(df[i:i + len_time].values))
    ...
    def _add_external_information_3d(self, df, ext_data=None):
        ...
        #     print('add_time_in_day and not is_time_nan')
        ...
        #     print('add_day_in_week and not is_time_nan')
        #     dayofweek = []
        ...
        region_embeddings = np.load(r"/xxx/KG_Embedding/data/NYC/Area_embedding.npy")


        last_column = region_embeddings[:, -1]

        sorted_indices = np.argsort(last_column)

        region_embeddings = region_embeddings[sorted_indices]

        region_embeddings_plus = np.zeros([260, 32]) #  same as the number as entities in file .geo
        for i in range(region_embeddings.shape[0]):
            region_embeddings_plus[i] = region_embeddings[i][:32]
        #
        ...
        # new_data = new_data[..., :2] # flow wo/E
        # new_data = new_data[..., :1] # classify wo/E
    ...
    def _split_train_val_test(self, x, y):
        ...
        if self.cache_dataset:
            ensure_dir(self.cache_file_folder)
            # s_new = s[:-5] + insert_text + s[-5:]

            np.savez_compressed(
                # self.cache_file_name[:-4] + '_wo' + self.cache_file_name[-4:],
    ...
    def get_data(self):
        ...
        print('>>>> raw  y_train 统计:', y_train[..., :self.output_dim].min(),
              y_train[..., :self.output_dim].max())
        #
        # 数据归一化
        ...
        print('>>>> scaled y_train 统计:', x_train[..., :self.output_dim].min(),
              x_train[..., :self.output_dim].max())
