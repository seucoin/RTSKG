...
def get_Station_embedding(grid_KG_id_path, entity_final_embedddings, val_stations, save_path):
    tmp = pd.read_csv(val_stations)
    valid_station_ids = set(tmp['geo_id'].unique())
    station = pd.read_csv(grid_KG_id_path)
    station_KG_id = station[["station_id", "KG_id", "Region_id"]].values
    filtered_embeddings = []
    count = 0
    for i in range(len(station_KG_id)):
        station_id = int(station_KG_id[i][0])
        kg_id = int(station_KG_id[i][1])
        region_id = int(station_KG_id[i][2])
    ...
    station_embeddings = np.array(filtered_embeddings, dtype=np.float16)
    np.save(save_path, station_embeddings)

# entity_final_embedddings, pca_final_embeddings, tsne_dim2_embeddings = get_embeddings(parser.parse_args())
# entity_final_embedddings = get_embedddings(parser.parse_args())
# np.save('./data/CHI/all_entity_embeddings.npy', entity_final_embedddings)
